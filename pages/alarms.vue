  ...
  <div>
    ...
    <div class="row">
      <div class="col-sm-12">
        ...
        <card v-else>
         Debes seleccionar un dispositivo para crear una alarma.
        </card>
      </div>
    </div>
    ...
    <div class="row" v-if="$store.state.devices.length > 0">
      <div class="col-sm-12">
        <card>
          <div slot="header">
            <h4 class="card-title">Lista de reglas</h4>
          </div>
          ...
          >
            ...
            <el-table-column prop="counter" label="Activaciones"></el-table-column>
          ...
          <h4 v-else class="card-title">Sin reglas de alarmas</h4>
        </card>
      </div>
    </div>
  </div>
        ...
        .then(res => {
          if (res.data.status == "success") {
            this.$notify({
              type: "success",
              icon: "tim-icons icon-check-2",
              message: "Estado de regla actualizada"
            });
    ...
    createNewRule() {
      if (this.selectedWidgetIndex == null) {
        this.$notify({
          type: "warning",
          icon: "tim-icons icon-alert-circle-exc",
          message: "Debe seleccionar una variable"
        });
        return;
      }
      ...
      if (this.newRule.triggerTime == null) {
        this.$notify({
          type: "warning",
          icon: "tim-icons icon-alert-circle-exc",
          message: " Tiempo de disparo vacío"
        });
        return;
      }
